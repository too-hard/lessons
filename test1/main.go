...
func (c *Conf) timer() {
	...
	*/
	...
	// в данном случае структура for будет ожидать, когда пройдёт c.Every секунд, после этого 
	// начнёт выполняться код в теле for
	for range time.After(time.Second * time.Duration(c.Every)){
		timeCount := time.Since(startTime).Seconds()
		fmt.Printf("время с запуска %v секунд\n", int(timeCount))
	}
}

// update - функция для перечитывания конфиг файла каждые c.Update сеекунд
func (c *Conf) update() {
	for range time.After(time.Second * time.Duration(c.Every)){
